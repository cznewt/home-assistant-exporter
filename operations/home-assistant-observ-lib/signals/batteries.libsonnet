...
function(this)
  local sig = mksig(this);
  {
    ...
    lowBattery: sig('Low batteries', 'count(hass_device_battery_remaining{%(queriesSelector)s} < ' + this.lowBatteryThreshold + ')', 'short', 'Devices below the low-battery threshold.'),
  }
